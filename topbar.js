...
// показывать старый кэш агента до ручного обновления страницы. Полная
// перезагрузка — самый надёжный способ сбросить И localStorage-производные
// данные, И всю JS-память страницы (переменные AGENT/CURRENT_AGENT и т.п.),
// точно так же, как это уже делает сама кнопка "Выйти из аккаунта".
